...
def GetNix() :
    try:
        ...
        print("Err Read Led")
        with open("/Nix.txt", "w") as bright:
            bright.write("0")
            bright.flush()
            Nixtype=0
            tNix="0"
            print ("Nix New=%s" % tNix)
...
try:
        if (typenix == 0) :
            Nixie_0 = displayio.OnDiskBitmap("/1/b00.bmp")
            Nixie_1 = displayio.OnDiskBitmap("/1/b01.bmp")
            Nixie_2 = displayio.OnDiskBitmap("/1/b02.bmp")
            Nixie_3 = displayio.OnDiskBitmap("/1/b03.bmp")
            Nixie_4 = displayio.OnDiskBitmap("/1/b04.bmp")
            Nixie_5 = displayio.OnDiskBitmap("/1/b05.bmp")
            Nixie_6 = displayio.OnDiskBitmap("/1/b06.bmp")
            Nixie_7 = displayio.OnDiskBitmap("/1/b07.bmp")
            Nixie_8 = displayio.OnDiskBitmap("/1/b08.bmp")
            Nixie_9 = displayio.OnDiskBitmap("/1/b09.bmp")
        if typenix == 1:
            Nixie_0 = displayio.OnDiskBitmap("/1/b00.bmp")
            Nixie_1 = displayio.OnDiskBitmap("/1/b01.bmp")
            Nixie_2 = displayio.OnDiskBitmap("/1/b02.bmp")
            ...
            Nixie_9 = displayio.OnDiskBitmap("/1/b09.bmp")
        if typenix == 2:
            Nixie_0 = displayio.OnDiskBitmap("/2/b00.bmp")
            Nixie_1 = displayio.OnDiskBitmap("/2/b01.bmp")
            Nixie_2 = displayio.OnDiskBitmap("/2/b02.bmp")
            Nixie_3 = displayio.OnDiskBitmap("/2/b03.bmp")
            Nixie_4 = displayio.OnDiskBitmap("/2/b04.bmp")
            Nixie_5 = displayio.OnDiskBitmap("/2/b05.bmp")
            Nixie_6 = displayio.OnDiskBitmap("/2/b06.bmp")
            Nixie_7 = displayio.OnDiskBitmap("/2/b07.bmp")
            Nixie_8 = displayio.OnDiskBitmap("/2/b08.bmp")
            Nixie_9 = displayio.OnDiskBitmap("/2/b09.bmp")

        if typenix == 3:
            Nixie_0 = displayio.OnDiskBitmap("/3/b00.bmp")
            Nixie_1 = displayio.OnDiskBitmap("/3/b01.bmp")
            Nixie_2 = displayio.OnDiskBitmap("/3/b02.bmp")
            Nixie_3 = displayio.OnDiskBitmap("/3/b03.bmp")
            Nixie_4 = displayio.OnDiskBitmap("/3/b04.bmp")
            Nixie_5 = displayio.OnDiskBitmap("/3/b05.bmp")
            Nixie_6 = displayio.OnDiskBitmap("/3/b06.bmp")
            Nixie_7 = displayio.OnDiskBitmap("/3/b07.bmp")
            Nixie_8 = displayio.OnDiskBitmap("/3/b08.bmp")
            Nixie_9 = displayio.OnDiskBitmap("/3/b09.bmp")
except OSError as e:  # Typically when the filesystem isn't writeable...
        print("Err Load Nix")            
...
display.show(group)
coloN=[(0, 0, 0),(250, 0, 0),(0, 250, 0),(0, 0, 250),(255, 255, 255)]
COLOR = (0, 0, 0) 
...
def GetLed() :
    try:
        ...
        print("Err Read Led")
        with open("/Led.txt", "w") as bright:
            bright.write("0")
            bright.flush()
            Ledtype=0
            tLed="0"
            print (tLed)
            print ("LED New=%s" % tLed)
...
def UpNix (tmpNix):
    ...
    if(tmpNix>0):
        ...
        Hour_label = label.Label(terminalio.FONT, color=0xFFFFFF, text = Hstr)
        Hour_label.anchor_point = (1.0, 0.0)
        Hour_label.anchored_position = (117, 45)
        Hour_label.scale = (5)
...
def UpMin (Mintmp):
    ...
    cs4.value = True
#**********************************************************#
#  Function SET Nixie Type                                 #
#**********************************************************#
def SettingNix(tmpNix):
    tNix =tmpNix
    SetOK=False
    #----------------------------------------------------------
    #LCD 1
...
# Draw a label
    text_group = displayio.Group(scale=2, x=1, y=90)
    text = "Type Nix>"
...
#Clear screen
    ...
    splash.append(bg_sprite)
# Draw a label
    text_group = displayio.Group(scale=5, x=17, y=85)
    if(tNix>0):
        text= f"{tNix:02n}"
    else :
        text= f"Orig" 
...
#            Buzz.value = False
    ...
    if btn_Mode.value:
        ...
        while(ModeOK == False):
            ...
            try:
                SettingDate(t.tm_year,t.tm_mon,t.tm_mday,t.tm_hour,t.tm_min)#(Yeartmp,Montmp,Daytmp,Hourtmp,Mintmp)
            except Exception:
                print('ERROR Date ')
            try:
                SettingYear(t.tm_year,t.tm_mon,t.tm_mday,t.tm_hour,t.tm_min)#(Yeartmp,Montmp,Daytmp,Hourtmp,Mintmp)
            except Exception:
                print('ERROR Year ')
            try:
                SettingAlarm(t.tm_year,t.tm_mon,t.tm_mday,t.tm_hour,t.tm_min)#(Yeartmp,Montmp,Daytmp,TmpH_alarm,TmpM_alarm)
            except Exception:
                print('ERROR Alarm ')
            try:
            ...
            SettingLED(Ledtype)
            Ledtype=int(GetLed())
            print ("LEDex=%s" % Ledtype)
        #---------------------------------------
            strtNix=GetNix()
            Nixtype=int(strtNix)
            tmpNix=Nixtype
            print ("tmpNix0=%s" % tmpNix)
            print ("Nixex0=%s" % Nixtype)        
            SettingNix(Nixtype)
            Nixtype=int(GetNix())
            print ("tmpNix=%s" % tmpNix)
            print ("Nixex=%s" % Nixtype)
            if(Nixtype!=tmpNix) :
                typenix = Nixtype
            else :    
                typenix = tmpNix #1
